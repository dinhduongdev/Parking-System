...
app.config["MONGO_URI"] = (
    "mongodb://localhost:27017/GiaiPhapPhanMem"  # Update with your MongoDB URI
)
mongo = PyMongo(app)
bcrypt = Bcrypt(app)
CORS(app)
            ...
            jsonify(
                {
                    ...
                    "username": user.get(
                        "username"
                    ),  # Ensure your user document has a 'username' field
                }
            ),
            200,
        )
    ...
    try:
        ...
        if user:
            ...
            current_balance = user.get(
                "balance", 0
            )  # Default to 0 if balance doesn't exist
            ...
            updated_user = users.find_one(
                {"username": username},
                {"_id": 0, "username": 1, "balance": 1, "license_plate": 1},
            )
...
def get_motorcycle_spots():
    spots = (
        mongo.db.motorcycle_spots.find()
    )  # Assuming you have a collection named 'motorcycle_spots'
    output = []
    for spot in spots:
        output.append(
            {
...
@app.route("/camera_in", methods=["GET"])
def get_user_of_plate():
    # Get the license plate from query parameters
    license_plate = request.args.get("license_plate")

    # Check if license plate was provided
    if not license_plate:
        return jsonify({"error": "License plate is required"}), 400

    # Find the user by license plate
    user = users.find_one({"license_plate": license_plate})
...
def get_qr_codes():
    ...
    user_data = data_collection.find_one(
        {"username": username}, {"_id": 0, "qr_codes": 1}
    )
