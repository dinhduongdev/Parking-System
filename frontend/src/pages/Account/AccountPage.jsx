...
const AccountPage = () => {
  ...
  const handleUpdatePlateSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/account/license_plate",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchQrCodes = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/get_qr_codes?username=${encodeURIComponent(
          user.username
        )}`,
        {
          method: "GET",
  ...
  const handleCreateQR = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/generate_qr?username=${encodeURIComponent(
          user.username
        )}&date=${encodeURIComponent(dateInVietnam)}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to generate QR code. Status: ${response.status}`
        );
      }
      ...
      const rechargeResponse = await fetch(
        "http://127.0.0.1:5000/account/vallet",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <main className="w-3/4 p-6">
        ...
        <section className="mt-6 p-4 bg-white shadow rounded">
          ...
          <p className="text-gray-700">
            Current Plate: {user?.license_plate || "Not Set"}
          </p>
          <button
